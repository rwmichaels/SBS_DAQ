...
unsigned int  csr_adc[]    = {5, 0, 1, 3, 7, 16};
unsigned int  csr_tdc72[]  = {2, 0, 1}; /*
unsigned int  csr_tdc75[]  = {2, 0, 1}; */
unsigned int  csr_tdc77[]  = {6, 0, 1, 3, 7, 16, 18}; /*
unsigned int  csr_tdc77S[] = {7, 0, 1, 3, 5, 7, 16, 18}; */
...
#endif
  unsigned long rslt, dmaptr=0, lenb, rb, rlen, len=65;
  int ii;
...
#endif
  lenb = len<<2;
  rslt = fb_frdb_1(slot,0,(unsigned int *)dmaptr,lenb,
		   (volatile unsigned int *)&rb,1,0,1,0,0x0a,0,0,1);
  if ((rb > (lenb+4))||(rslt != 0)) {
...
char *fb_72_75_stats (int slot, int csr0, char b_type[10], int prn)
{
  unsigned int csr0_back, csr1, csr1_II;
...
char *fb_77_stats (int slot, int csr0, char b_type[10], int prn)
{
  unsigned int  csr1, csr18, csr0_t, csr1_t, csr18_t;
  unsigned int  csr16, wbuf, rbuf;
  int  bit;
  char *info;
...
char *fb_81_stats (int slot, int csr0, char b_type[10], int prn)
{
  unsigned int  csr1, csr16, wbuf, rbuf;
  int  bit;
  char *info;
...
{
  int  slot;
  unsigned int csr_tmp, i_csr, *csr;
